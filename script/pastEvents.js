...
async function iniciar() {
    ...
    filtrarCateg(eventosPasados)



    let categoriesDiv = document.querySelector("#categorias-p");
    ...
    const divEventosPasados = document.getElementById("past-events");
    ...
    checkboxes.forEach((checkbox) => {
        checkbox.addEventListener('change', () => {
            ...
            const filteredObjects = eventosPasados.filter((obj) => {
    ...
    function mostrar(contenido) {
        divEventosPasados.innerHTML = "";
        if (contenido.length > 0) {
            imprimir(contenido)
        }
        else {
            imprimir(eventosPasados)
        }
        divEventosPasados.appendChild(fragmento);
    }
    ...
    function buscar() {
        ...
        if (resultados.length === 0) {
            divEventosPasados.innerHTML = "<p>No se encontraron resultados para su búsqueda. Por favor intente de nuevo.</p>";
        } else {
            mostrar(resultados);
